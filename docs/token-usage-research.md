...
> 决定时间：v1.6.0 前后。下面整节是决策依据，其余章节是继续开发时要用的技术细节。

方案是通的（见 §5），复杂度也不高，**卡住的是风险与收益不对称**：

| | 说明 |
| --- | --- |
| 收益 | 对话 footer 上多一行「输入 / 输出 token · 缓存命中率」 |
| 代价 | 有几率把 **12.4MB 的 agent 核心**写坏，届时 Kiro 的 agent 整体不可用 |

对比已上线的「对话面板实时耗时」补丁：那个改的是 **700 字节的 loader**，写坏了只是面板
不好使；这个写坏了是整个 agent 不能用，而且往往在你正要用的时候才发现。

各环节的复杂度评估（供将来估工时）：

| 维度 | 评估 |
| --- | --- |
| 定位代码 | **低**。已在真实文件验证唯一命中，锚点按语义字符串反解 minified 名 |
| 注入内容 | **低**。一条语句，整句包在 `try{}catch{}` 里 |
| 回传通道 | **低**。`appendFile` 异步 + 空回调，一行 JSON |
| 关联到轮 | **低**。RequestId ⨝ `usage_summary.requestIds[]`，数据现成 |
| 抗升级 | **中**。Kiro 升级覆盖后可按既有机制自愈重打；它重构这段代码则锚点失效 |
| **改动目标** | **高**。见上表 |

若将来重启这件事，**第一步应该是只插一句写 `alive` 的最小标记补丁**（见 §5.5），
用几乎为零的风险验证「锚点对了 + 补丁被加载 + 文件通道通了」三件事，再谈真回调。
