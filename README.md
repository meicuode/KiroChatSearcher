...
计时就再也不显示了。所以横条每次刷新（200ms）都重新在文档里找。

#### 横条属于哪个会话：走 React fiber 精确认人

Kiro 的 DOM 上没有任何会话标记，但它是 React 应用，而 React 把内部指针挂成 DOM 节点的
**自有属性**（属性名带一个进程内随机后缀，只能扫前缀、不能写死）：
...
会话身份在组件树里是显式传递的——每个会话一个自己的 store，外面还包着一个以
`sessionId` 为 prop 的 provider：

```js
cQ = a(t => ly((e, n) => ({ sessionId: t, session: [], appendToSession: … })))
```

于是从横条的 fiber 沿 `return` 往上走，找第一个能取出 `sessionId` 的祖先
（`memoizedProps.sessionId`，或 context value 里那个 per-session store 的
`getState().sessionId`）。这个配对是**精确**的，与同时跑几个会话无关。
全程只读、包在 try 里，失效不影响 Kiro。

**解析不出来时不猜**：只有在完全无歧义（没配上的轮恰好一个、身份不明的横条也恰好一条）
时才配；否则放弃横条这个落点。宁可不显示，也不显示一个可能配反的数字——显示错的比不显示
更糟，因为你没法判断它对不对。诊断字段 `__kcsTurnTimer` 里的 `fiberHits` /
`fallbackPairs` / `ambiguous` / `barSessions` 就是看这条路走到哪一档。

落点优先级：

1. 本会话的横条（fiber 精确配对，或无歧义时的兜底配对）；
2. 一条横条都没有、而捕获到的会话视图还活着 → 退到该会话自己的消息流末尾（Kiro 换了
   横条类名时的兜底。用的是**发起时捕获的那棵**，所以仍然不会串到别的会话去）；
3. 连会话视图都没捕获到 → 右下角浮动。

配上横条后会顺手把捕获的会话视图元素刷新成当前这一棵（`reacquired` +1），
所以第 2 条在会话视图被重建之后依然可用。

两种挂载模型都成立：隐藏的会话视图**留在 DOM 里**时它的横条也还在，计时留在那条
...
已知边界：横条解析出的会话不在本 webview 的轮记录里（例如同一会话同时开在侧边栏和
编辑器分栏、而那一轮是另一边发起的），这条横条就不用——不是我们的，不往里写。
...
并能模拟「切换会话 = 整棵重建」、以及给横条挂上带 `sessionId` 的 React fiber 替身
（含「fiber 取不到」的那一档）。共 37 条，上面每个 bug 都有一组回归钉子。

这些断言不是只对当前实现自说自话：`SCRIPT_PATH` 支持用环境变量 `KCS_TURN_TIMER_SCRIPT`
指向别的副本，把它指到出过 bug 的历史版本会红——

| 历史版本 | 红几条 | 说明 |
| --- | --- | --- |
| 「所有在途请求都收到响应才收工」 | 19 | 中断后计时停不下来 |
| 「记住会话视图元素」 | 4 | 换行落点 2 + 切走再切回 2 |
| 「可见优先 + 最新优先」启发式配对 | 7 | 并行会话数字可能配反、有歧义时未放弃显示 |

> **替身不能代替真机验证。** 这套测试跑的是我们自己写的 DOM 替身，而前两轮 bug 的成因
> 恰恰是对 Kiro 真实 DOM 的假设错了（以为切换会话只是隐藏视图，实际是整棵重建）。
> 建立在错误假设上的替身再全绿也证明不了真实行为，所以改完必须在真实 Kiro 里过一遍，
> 再提交打 tag。
...
- **点这个按钮直接打开那个会话**（多个会话在等时打开事件时间最新的那个）
- 确认后自动还原
...
看到提示之后你要做的事就是去处理它，所以按钮把你直接送到那儿（此前点它是切换搜索
侧边栏，和「谁在等我确认」没有关系）。也可以从命令面板走：`Kiro: 打开待确认的对话`
（`kiroChatSearch.openAttentionSession`）。

目标的选取规则与按钮上显示的文字同一优先级：

- **待确认优先于已完成** —— 前者卡着等你点，后者只是有结果了
- 多个会话同时在等时取 **`pending_interaction` 事件时间最新**的那条；时间戳缺失
  （外部进程写的文件，任何字段都可能没有）时带时间戳的一律优先，都没有则取文件里
  出现得更晚的那条
- 只有「已完成」时打开**刚跑完的那个会话**
- 点下去时状态已经变了（在别处确认过）→ 状态栏闪一条提示并立刻重扫，不弹窗打断

会话身份（= `NewSessionDir` 的目录名，也就是跳转命令要的 sessionId）**不在
`messages.jsonl` 的事件里**，它来自文件所在的目录名。所以解析函数
`scanPendingInteractions` 产出的项不带 `sessionId`，由 `AttentionWatcher` 在枚举目录时
盖上——谁知道就由谁填，不在解析层假装知道。

跳转复用 `openSessionByJump`，与搜索结果、排行页标题走同一条候选链，所以这一侧
不需要认识任何 `kiroAgent.*` 命令。
